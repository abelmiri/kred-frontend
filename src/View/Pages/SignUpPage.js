                ...
                if (level === 1)
                {
                    api.post("code", {phone: this.phoneInput.value}, "")
                        ...
                        .catch(e => this.setState({...this.state, loading: false}, () =>
                        {
                            ...
                            else if (e?.response?.status === 400 && e?.response.data.message === "you get it many time!")
                            {
                                this.setState({...this.state, level: 2, loading: false}, () => NotificationManager.warning("کد 3 بار برای شما فرستاده شده است!"))
                            }
